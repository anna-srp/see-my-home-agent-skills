...
const statePath = path.join(stateDir, 'see-my-home-agent.json')
...
const labels = {
  app: 'see-my-home',
  role: 'residential-design-visualizer',
...
if (!agent) {
  ...
  }, 'see-my-home-design-agent-v1')
  source = 'created'
  agent = await zc.getAgent(agent.agent_id)
} else {
  const declared = agent.declared ?? {}
  const updates = {}
  if (declared.name !== 'See My Home') updates.name = 'See My Home'
...
for (const local of await Promise.all(skillNames.map(packageSkill))) {
  ...
  if (!skill) {
    skill = await zc.uploadSkill(await readFile(local.zipPath), {
      ...
      idempotencyKey: `see-my-home-${local.name}-v1`,
    })
    ...
    const createdVersion = await zc.uploadSkillVersion(skill.skill_id, await readFile(local.zipPath), {
      fileName: `${local.name}.zip`,
      idempotencyKey: `see-my-home-${local.name}-${local.hash.slice(0, 16)}`,
    })
